...
const { loadConfig, resolveModelInternal, findPhaseInternal, getRoadmapPhaseInternal, pathExistsInternal, generateSlugInternal, getMilestoneInfo, getMilestonePhaseFilter, stripShippedMilestones, extractCurrentMilestone, normalizePhaseName, planningPaths, planningDir, planningRoot, planningBaseName, toPosixPath, output, error, checkAgentsInstalled } = require('./core.cjs');
...
function cmdInitNewProject(cwd, raw) {
  ...
  try {
    ...
    const skipDirs = new Set(['node_modules', '.git', '.planning', planningBaseName(cwd), '.claude', '__pycache__', 'target', 'dist', 'build']);
  ...
  const result = {
    ...
    // Existing state
    project_exists: pathExistsInternal(cwd, planningBaseName(cwd) + '/PROJECT.md'),
    has_codebase_map: pathExistsInternal(cwd, planningBaseName(cwd) + '/codebase'),
    planning_exists: pathExistsInternal(cwd, planningBaseName(cwd)),

    // Brownfield detection
    has_existing_code: hasCode,
    has_package_file: hasPackageFile,
    is_brownfield: hasCode || hasPackageFile,
    needs_codebase_map: (hasCode || hasPackageFile) && !pathExistsInternal(cwd, planningBaseName(cwd) + '/codebase'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    ...
    // File existence
    planning_exists: pathExistsInternal(cwd, planningBaseName(cwd)),
    codebase_dir_exists: pathExistsInternal(cwd, planningBaseName(cwd) + '/codebase'),
  };
...
function cmdInitManager(cwd, raw) {
  ...
  try {
    const waitingPath = path.join(cwd, planningBaseName(cwd), 'WAITING.json');
...
function cmdInitListWorkspaces(cwd, raw) {
  ...
  if (fs.existsSync(defaultBase)) {
    ...
    for (const entry of entries) {
      ...
      hasProject = fs.existsSync(path.join(wsPath, planningBaseName(wsPath), 'PROJECT.md'));
