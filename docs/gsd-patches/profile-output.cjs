...
const os = require('os');
const { output, error, safeReadFile, planningBaseName } = require('./core.cjs');
...
function generateProjectSection(cwd) {
  const projectPath = path.join(cwd, planningBaseName(cwd), 'PROJECT.md');
...
function generateStackSection(cwd) {
  const codebasePath = path.join(cwd, planningBaseName(cwd), 'codebase', 'STACK.md');
  const researchPath = path.join(cwd, planningBaseName(cwd), 'research', 'STACK.md');
...
function generateConventionsSection(cwd) {
  const conventionsPath = path.join(cwd, planningBaseName(cwd), 'codebase', 'CONVENTIONS.md');
...
function generateArchitectureSection(cwd) {
  const architecturePath = path.join(cwd, planningBaseName(cwd), 'codebase', 'ARCHITECTURE.md');
