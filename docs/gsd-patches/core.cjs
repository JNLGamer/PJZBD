...
// ─── Subject-aware planning dir name (PJZBD local patch) ──────────────────────
//
// Allows running multiple parallel projects in the same workspace with separate
// planning state. Resolves a "subject" name (alphanumeric / dash / underscore)
// from one of:
//   1. process.env.GSD_SUBJECT          (per-session, parallel-safe)
//   2. <cwd>/.gsd-subject file          (persistent default for a workspace)
// When a subject is set, the planning base directory becomes ".planning-<subject>".
// When unset, behavior is identical to vanilla GSD (".planning").
//
function _gsdSubject(cwd) {
  let s = process.env.GSD_SUBJECT;
  if (!s && cwd) {
    try {
      const f = path.join(cwd, '.gsd-subject');
      if (fs.existsSync(f)) s = fs.readFileSync(f, 'utf-8').trim();
    } catch { /* ignore */ }
  }
  if (!s) return null;
  if (!/^[A-Za-z0-9_-]+$/.test(s)) return null; // sanitize: reject paths/dots
  return s;
}
function planningBaseName(cwd) {
  const s = _gsdSubject(cwd);
  return s ? `.planning-${s}` : '.planning';
}
...
function findProjectRoot(startDir) {
  ...
  const ownPlanning = path.join(resolved, planningBaseName(resolved));
  ...
  while (dir !== root) {
    ...
    const parentPlanning = path.join(parent, planningBaseName(parent));
...
function loadConfig(cwd) {
  const configPath = path.join(cwd, planningBaseName(cwd), 'config.json');
  const defaults = {
...
function resolveWorktreeRoot(cwd) {
  ...
  if (fs.existsSync(path.join(cwd, planningBaseName(cwd)))) {
    return cwd;
  }
...
function planningDir(cwd, ws) {
  ...
  const base = planningBaseName(cwd);
  if (!ws) return path.join(cwd, base);
  return path.join(cwd, base, 'workstreams', ws);
}

/** Always returns the root .planning/ path, ignoring workstreams. For shared resources. */
function planningRoot(cwd) {
  return path.join(cwd, planningBaseName(cwd));
}
...
function planningPaths(cwd, ws) {
  ...
  const root = path.join(cwd, planningBaseName(cwd));
  return {
    planning: base,
...
function findPhaseInternal(cwd, phase) {
  ...
  const milestonesDir = path.join(cwd, planningBaseName(cwd), 'milestones');
  ...
  try {
    ...
    for (const archiveName of archiveDirs) {
      ...
      const relBase = planningBaseName(cwd) + '/milestones/' + archiveName;
    ...
    for (const archiveName of phaseDirs) {
      ...
      for (const dir of dirs) {
        results.push({
          name: dir,
          milestone: version,
          basePath: path.join(planningBaseName(cwd), 'milestones', archiveName),
