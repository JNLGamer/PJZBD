...
const { escapeRegex, getMilestonePhaseFilter, extractOneLinerFromBody, normalizeMd, planningPaths, planningBaseName, output, error } = require('./core.cjs');
...
function cmdMilestoneComplete(cwd, version, options, raw) {
  ...
  const milestonesPath = path.join(cwd, planningBaseName(cwd), 'MILESTONES.md');
  const archiveDir = path.join(cwd, planningBaseName(cwd), 'milestones');
  ...
  const auditFile = path.join(cwd, planningBaseName(cwd), `${version}-MILESTONE-AUDIT.md`);
